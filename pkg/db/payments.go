...
func ListPayments(ctx context.Context, pool *pgxpool.Pool, playerID, matchID *int) ([]models.Payment, error) {
	query := `SELECT pay.id, pay.player_id, pl.name, pay.match_id, pay.amount, pay.notes, pay.paid_at::text, pay.created_at
	          FROM payments pay JOIN players pl ON pl.id=pay.player_id WHERE 1=1`
	args := []any{}
	idx := 1

	if playerID != nil {
		query += fmt.Sprintf(" AND pay.player_id=$%d", idx)
		args = append(args, *playerID)
		idx++
	}
	if matchID != nil {
		query += fmt.Sprintf(" AND pay.match_id=$%d", idx)
		args = append(args, *matchID)
		idx++
	}
	query += " ORDER BY pay.paid_at DESC, pay.id DESC"
	...
	for rows.Next() {
		...
		if err := rows.Scan(&p.ID, &p.PlayerID, &p.PlayerName, &p.MatchID, &p.Amount, &p.Notes, &p.PaidAt, &p.CreatedAt); err != nil {
			return nil, err
		}
...
func CreatePayment(ctx context.Context, pool *pgxpool.Pool, playerID int, matchID *int, amount float64, paidAt string, notes *string) (*models.Payment, error) {
	...
	err := pool.QueryRow(ctx,
		`INSERT INTO payments (player_id, match_id, amount, paid_at, notes) VALUES ($1,$2,$3,$4,$5)
		 RETURNING id, player_id, (SELECT name FROM players WHERE id=player_id), match_id, amount, notes, paid_at::text, created_at`,
		playerID, matchID, amount, paidAt, notes).
		Scan(&p.ID, &p.PlayerID, &p.PlayerName, &p.MatchID, &p.Amount, &p.Notes, &p.PaidAt, &p.CreatedAt)
	if err != nil {
